...
static Octstr* find_configuration(char* binary);
...
int main(int argc, char *argv[])
{
     ...
     if (argv[cfidx] == NULL) {
		fname = find_configuration(argv[0]); // find configuration relative to the binary
	...
	/* Don't complain about this, as we will try to extract from email
     if (!xto || !xfrom) {
	...
	*/	
	...
	/* Get to and from in headers */
	if (!xto) 
		xto = http_header_value(headers, octstr_imm("To"));
	if (!xfrom)
		xfrom = http_header_value(headers, octstr_imm("From"));
...
static Octstr* find_configuration(char* name) {
	char *base, *instance;
	char buf[1024];
	char path_buf[PATH_MAX+1];
	getcwd(path_buf, PATH_MAX+1);
	...
	for (instance = base; *instance; instance++)
		if ('.' == (*instance)) {
			instance++;
			break;
		}
	...
	sprintf(buf, "%s/../etc/mbuni.%s.conf", path_buf, instance);
	return octstr_imm(buf);
}
