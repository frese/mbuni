...
int mms_validate_address(Octstr *s)
{
     ...
     if (octstr_search_char(s, '@', 0) > 0)
	  ...
	  if (octstr_case_search(s, octstr_imm("PLMN"), 0) + 4 == l ||
		  ...
		  octstr_case_search(s, octstr_imm("unknown"), 0) + 7 == l)
	       return 0;
	  else
	       return -1;
     else
...
static int encode_msgheaders(Octstr *os, List *hdrs)
{
     ...
     for (i = 0; fcont && i < l; i++) {
	  ...
	  if(octstr_ncompare(octstr_imm("X-Mbuni"), field, 7)==0)
		goto loop1;
...
void mms_msgdump(MmsMsg *m, int headers_only)
{
     ...
     debug("mms.dump", 0, "Dumping MMS message body (%s) [%ld parts] --> ", 
	   m->ismultipart ? "multipart" : "not multipart", 
...
MmsMsg *mms_frommime(MIMEEntity *mime)
{
	...
	Octstr *s;
	MIMEEntity *mx;
	int i, n, msg_include_smil;
	List *h;

	if (!mime)
		return NULL;

	mime_entity_dump(mime);
	...
	if ((n = mime_entity_num_parts(mx)) > 0) {
		...
		for (i = 0; i < n; i++) {
			Octstr *type,*charset;
			MIMEEntity* body_part = mime_entity_get_part(mx, i);
			List *ent_headers = mime_entity_headers(body_part);
			http_header_get_content_type(ent_headers, &type, &charset);
			debug("mms_frommime",0,"comparing content type [%s]", octstr_get_cstr(type));
			if (octstr_case_search(type,octstr_imm("application/smil"),0)==0)
				msg_include_smil = 1;
			gwlist_append(m->body.l, body_part);
			octstr_destroy(type);
			octstr_destroy(charset);
			gwlist_destroy(ent_headers,octstr_destroy_item);
		}
	} else {
		m->ismultipart  = 0;	  
	...
	for (i = 0; i<gwlist_len(m->headers); i++) {
		...
		if (mms_is_token(name)) { /* if header name is bad, kill this header field. */
			debug("mms_frommime",0,"mms_from_mime: name=%s,value=%s",octstr_get_cstr(name),octstr_get_cstr(value));
			if ((octstr_case_compare(name,octstr_imm("Content-Type"))==0) &&
				(octstr_case_search(value, octstr_imm("multipart/"), 0) == 0)) {
				if (msg_include_smil)
					http_header_add(h,"Content-Type", "application/vnd.wap.multipart.related");
				else
					http_header_add(h,"Content-Type", "application/vnd.wap.multipart.mixed");
			} else {
