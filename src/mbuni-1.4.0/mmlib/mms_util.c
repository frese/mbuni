...
int mms_load_core_settings(mCfgGrp *cgrp)
{
     ...
     if (log != NULL) {
       if (mms_cfg_get_int(cgrp, octstr_imm("log-level"), &loglevel) == -1)
	       loglevel = 0;
       log_rotate = mms_cfg_get(cgrp, octstr_imm("log-rotate"));
     ...
     if (alog) {
          log_rotate = mms_cfg_get(cgrp, octstr_imm("access-log-rotate"));
          log_rotate = log_rotate ? log_rotate : octstr_imm(""); 
	  alog_open(octstr_get_cstr(alog), 1, 1, octstr_get_cstr(log_rotate));
	  octstr_destroy(alog);
          octstr_destroy(log_rotate);
     }
...
void mms_log_service(char *logmsg, Octstr *from, List *to, 
	Octstr *msgid, Octstr *keyword, Octstr *service, char *fetch_result)
{
     Octstr *xto = octstr_create("");
     int i, n = to ? gwlist_len(to) : 0;
     ...
     alog("%s MMS [from:%s] [to:%s] [msgid:%s] [Keyword:%s] [Service:%s] [Dispatch-status:%s]",
	  logmsg,
	  from ? octstr_get_cstr(from) : "",
	  octstr_get_cstr(xto),
	  msgid ? octstr_get_cstr(msgid) : "",
	  keyword ? octstr_get_cstr(keyword) : "",
	  service ? octstr_get_cstr(service) : "",
	  fetch_result
	 );
...
void release_file_lock(int fd, file_key *key) {
	debug("mm_util", 0, "unlocking inode %d", key->inode);
    ...
    if (item && item->fd == fd) {
		...
		warning(0, "Attempt to unlock file which isn't locked!");
	}
    debug("mm_util",0,"unlocked inode %d", key->inode);
...
int sun_lockfile(int fd, int shouldblock)
{
    ...
    debug("mm_util",0,"Trying to lock inode %d", key.inode);
    pthread_mutex_lock(&listMutex);
...
#ifdef SunOS
static int reserve_file(int fd) {
	int ret; 
	fshare_t reservation;

	reservation.f_access = F_RWACC;
    reservation.f_deny = F_RWDNY;
	reservation.f_id = gwthread_self();
	
	ret = fcntl(fd, F_SHARE, reservation);
	debug("",0,"reserve_file file=%d thread=%ld ret=%d", fd, gwthread_self(),ret);
...
{
#ifdef SunOS
	int ret = sun_lockfile(fd,shouldblock);
#else
     ...
     if (ret != 0 ||  (ret = check_lock(fd,fname)) != 0)
	  return ret;     
     return 0;
}
...
void _mms_fixup_address(Octstr **address, char *unified_prefix, List *strip_prefixes, int keep_suffix)
{
     ...
     if (isphonenum(*address) || (typ && octstr_str_compare(typ, "/TYPE=PLMN") == 0)) {
	  ...
	  octstr_append(*address, keep_suffix ? octstr_imm("/TYPE=unknown") : octstr_imm(""));     
     octstr_destroy(typ);
}
		  ...
		  List **cgivars, List **cgivar_ctypes)
{
	 ...
	 //info(0, "ctype is %s", octstr_get_cstr(ctype));
	...
	if (octstr_search(url, octstr_imm("data:"), 0) == 0) {
		...
		*reply_headers = http_create_empty_headers();
		...
		if (status == HTTP_UNAUTHORIZED) { 
			...
			status = fetch_url_with_auth(c, method, url, request_headers, body, v, reply_headers, reply_body);
			octstr_destroy(v);
		}
...
int mms_url_fetch_content_with_basic_auth(int method, Octstr *url, List *request_headers, 
			       Octstr *body, List **reply_headers, Octstr **reply_body) {
	HTTPCaller *c = http_caller_create();
	fetch_url_with_auth(c, method, url, request_headers, body, octstr_imm("WWW-Authenticate: Basic"), reply_headers, reply_body);
	http_caller_destroy(c);
}
			       ...
			       Octstr *body, Octstr *auth_hdr,  List **reply_headers, Octstr **reply_body)
{
	...
	if ((i = octstr_search_char(xauth_value, ' ', 0)) < 0) {
     ...
     auth_type = octstr_copy(xauth_value, i+1, octstr_len(xauth_value));
     //octstr_delete(xauth_type, 0, i+1);

	debug("",0,"auth_type is %s", octstr_get_cstr(auth_type));

     if (octstr_str_case_compare(auth_type, "Basic") == 0) {
		debug("",0,"performing Basic authentication");
		Octstr *tmp = octstr_format("%S:%S", h->user, h->pass ? h->pass : octstr_imm(""));
		octstr_binary_to_base64(tmp);
		x = octstr_format("Digest %S", tmp);
		octstr_destroy(tmp);
		goto almostdone;
     } /* else digest. */
